  ...
  return (
    <>
      <Card.Group itemsPerRow={1} stackable={true} doubling={true}>
        {Object.keys(address.polygonliquidityPools).map((poolname) => (
          <Card key={poolname} className="fluid">
            ...
            {address.polygonliquidityPools[poolname].map((blockKey) => (
              <Card.Content extra>
                <Grid columns='two' divided>
                  <Grid.Row>
                    <Grid.Column width={5}>
                      <Grid.Row style={{ fontWeight: "600", fontSize: "16px", marginBottom: "10px", color:"#000" }}> {address.polygonBlocks[blockKey.block].platform.tname.split('-')[1]+" Liquidity" || "Not Found"} </Grid.Row>
                      <Grid.Row style={{ fontWeight: "400", fontSize: "16px", marginBottom: "10px", color: "#000" }}> {`${Moment((address.polygonBlocks[blockKey.block].transactions[0].timeStamp * 1000)).format('YYYY-MM-DD hh:mm:ss a')}`} </Grid.Row>
                      <Grid.Row style={{ fontWeight: "600", fontSize: "16px", marginBottom: "10px" }}> <a style={{ color: "#4183c4" }} href={"https://etherscan.io/tx/" + address.polygonBlocks[blockKey.block].transactions[0].hash} target="_blank"> {`Block - ${blockKey.block}`} </a> </Grid.Row>
                      {(address.polygonBlocks[blockKey.block].in.length > 0 && address.polygonBlocks[blockKey.block].out.length > 0)
                        ? <Grid.Row style={{ fontWeight: "600", fontSize: "16px", marginTop: "10px", marginBottom: "20px" }}>
                          <Grid columns='two' divided>
                            <Grid.Row>
                              <Grid.Column>
                                {address.polygonBlocks[blockKey.block].out.map((trr, keyr) => (
                                  <Grid.Row>
                                    {(trr.tokenValue > 0) ? (trr.tokenValue / Math.pow(10, trr.tokenDecimal || 18)).toFixed(2) + ' ' + (trr.tokenLabel || trr.tokenSymbol || 'MATIC ') : ''}
                                  </Grid.Row>
                                ))}
                              </Grid.Column>
                              <Grid.Column>
                                {address.polygonBlocks[blockKey.block].in.map((trr, keyr) => (
                                  <Grid.Row>
                                    {(trr.tokenValue > 0) ? (trr.tokenValue / Math.pow(10, trr.tokenDecimal || 18)).toFixed(2) + ' ' + (trr.tokenLabel || trr.tokenSymbol || 'MATIC ') : ''}
                                  </Grid.Row>
                                ))}
                      ...
                      <Grid.Row>
                        ...
                        {`${cumuGasFee(parseFloat(address.polygonBlocks[blockKey.block].gasFee))} MATIC`}
                      </Grid.Row>
                    </Grid.Column>
                    <Grid.Column width={11}>
                      <Card.Content extra>
                        {address.polygonBlocks[blockKey.block].transactions.map((tr, key) => (
                          <Grid columns='three' divided>
                            <Grid.Row>
                              ...
                              <Grid.Column>
                                ...
                                {(tr.value == 0) ? " " : (tr.tokenSymbol || 'MATIC ')}
...
export default PolygonLiquidityPoolsPane