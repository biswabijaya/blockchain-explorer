...
const EtheriumTokenTransfersPane = ({ address }) => {
  // console.log(address.etheriumTokenTransfers.length);
  var i=1;
  if (!address.etheriumTokenTransfers) return null
  return (
    <Table celled>
      <Table.Header>
      ...
      <Table.Body>
        {address.etheriumTokenTransfers.map((tr, key) => (
          <Table.Row key={key}>
            <Table.Cell width={4}>
...
export default EtheriumTokenTransfersPane