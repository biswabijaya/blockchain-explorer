      ...
      <Card.Group itemsPerRow={1} stackable={true} doubling={true}>
        {Object.keys(address.etheriumliquidityPools).map((poolname) => (
          <Card key={poolname} className="fluid">
            ...
            {address.etheriumliquidityPools[poolname].map((blockKey) => (
              <Card.Content extra>
                <Grid columns='two' divided>
                  <Grid.Row>
                    <Grid.Column width={5}>
                      <Grid.Row style={{ fontWeight: "600", fontSize: "16px", marginBottom: "10px", color:"#000" }}> {address.etheriumBlocks[blockKey.block].platform.tname.split('-')[1]+" Liquidity" || "Not Found"} </Grid.Row>
                      <Grid.Row style={{ fontWeight: "400", fontSize: "16px", marginBottom: "10px", color: "#000" }}> {`${Moment((address.etheriumBlocks[blockKey.block].transactions[0].timeStamp * 1000)).format('YYYY-MM-DD hh:mm:ss a')}`} </Grid.Row>
                      <Grid.Row style={{ fontWeight: "600", fontSize: "16px", marginBottom: "10px" }}> <a style={{ color: "#4183c4" }} href={"https://etherscan.io/tx/" + address.etheriumBlocks[blockKey.block].transactions[0].hash} target="_blank"> {`Block - ${blockKey.block}`} </a> </Grid.Row>
                      {(address.etheriumBlocks[blockKey.block].in.length > 0 && address.etheriumBlocks[blockKey.block].out.length > 0)
                        ? <Grid.Row style={{ fontWeight: "600", fontSize: "16px", marginTop: "10px", marginBottom: "20px" }}>
                          <Grid columns='two' divided>
                            <Grid.Row>
                              <Grid.Column>
                                {address.etheriumBlocks[blockKey.block].out.map((trr, keyr) => (
                                  <Grid.Row>
                              ...
                              <Grid.Column>
                                {address.etheriumBlocks[blockKey.block].in.map((trr, keyr) => (
                                  <Grid.Row>
                                    {(trr.tokenValue > 0) ? (trr.tokenValue / Math.pow(10, trr.tokenDecimal || 18)).toFixed(2) + ' ' + (trr.tokenLabel || trr.tokenSymbol || 'Ether ') : ''}
                                  </Grid.Row>
                                ))}
                      ...
                      <Grid.Row>
                        ...
                        {`${cumuGasFee(parseFloat(address.etheriumBlocks[blockKey.block].gasFee))} Eth`}
                      </Grid.Row>
                    </Grid.Column>
                    <Grid.Column width={11}>
                      <Card.Content extra>
                        {address.etheriumBlocks[blockKey.block].transactions.map((tr, key) => (
