...
const PolygonTokenTransfersPane = ({ address }) => {
  // console.log(address.polygonTokenTransfers.length);
  var i = 1;
  if (!address.polygonTokenTransfers) return null
  return (
    <Table celled>
      <Table.Header>
      ...
      <Table.Body>
        {address.polygonTokenTransfers.map((tr, key) => (
          <Table.Row key={key}>
            <Table.Cell width={4}>
...
export default PolygonTokenTransfersPane